...
'''def check_resource(re, msg, request1):
	request = request1
	while True:
		for r in resources:
			if re != r['name'] and re.startswith(r['name'] + "/"):
				handleMessage(msg)
				return'''
		...
		if message['type'] == 'demand':
			for r in resources:
				if message['resource'] == r or message['resource'].startswith(r + "/"):
					...
					#threading.Thread(target=check_resource(r['name'], message, request)).start()
					return
			...
			send(message['resource'] + " is now under your control")
			#check_timeout(message['resource'])
			#threading.Thread(target=check_timeout(resources[resources.index(re)])).start()
...
def test_connect():
	...
	clients[request.sid] = []
	print(request.sid + 'has entered the room.')
...
'''def check_timeout(r):
	...
	while True:
		...
		#for r in resources:
		print("here")
		print(time.time() - r['last_time'])
		if time.time() - r['last_time'] > 10:
			print("Here we go")
			for s in clients:
				for c in s:
					if c == r['name']:
